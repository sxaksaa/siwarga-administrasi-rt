...
function BillingHistory({ bills = [] }) {
  return <div className="history-section billing-history"><h4>Riwayat Tagihan & Pembayaran</h4><div className="table-wrap"><table><thead><tr><th>Periode</th><th>Penghuni</th><th>Iuran</th><th>Nominal</th><th>Terbayar</th><th>Status</th></tr></thead><tbody>{bills.map(bill=><tr key={bill.id}><td>{bill.periode_tagihan}</td><td><strong>{bill.nama_penghuni}</strong></td><td>{bill.jenis_iuran}</td><td>{rupiah(bill.nominal)}</td><td>{rupiah(bill.nominal_terbayar)}</td><td><span className={`badge ${bill.status}`}>{bill.status.replace('_',' ')}</span></td></tr>)}{!bills.length&&<tr><td colSpan="6" className="empty">Belum ada riwayat tagihan.</td></tr>}</tbody></table></div></div>
}
...
export default function Houses() {
  ...
  return <div className="page-stack">
    ...
    {detail&&<div className="modal-layer"><div className="modal modal-large"><div className="modal-head"><div><span className="eyebrow">DETAIL RUMAH</span><h3>{detail.nomor_rumah}</h3></div><button onClick={()=>setDetail(null)}><X/></button></div>{error&&<div className="alert error modal-alert">{error}</div>}<div className="detail-summary"><div><small>STATUS</small><span className={`badge ${detail.status}`}>{detail.status==='dihuni'?'Dihuni':'Tidak dihuni'}</span></div><div><small>ALAMAT</small><strong>{detail.alamat||'-'}</strong></div><div><small>PENGHUNI AKTIF</small><strong>{detail.penghuni_aktif?.penghuni?.nama_lengkap||'Belum ada'}</strong>{detail.penghuni_aktif?.penghuni?.jenis_penghuni&&<span className={`badge detail-resident-type ${detail.penghuni_aktif.penghuni.jenis_penghuni}`}>{detail.penghuni_aktif.penghuni.jenis_penghuni}</span>}</div></div>{detail.status==='tidak_dihuni'?<button className="button primary" onClick={()=>{setOccupancyForm({penghuni_id:'',mulai_tinggal:today,catatan:''});setError('');setOccupancyModal(true)}}><UserPlus size={17}/>Tempatkan Penghuni</button>:<div className="end-occupancy"><label>Tanggal keluar<input type="date" value={endDate} min={detail.penghuni_aktif?.mulai_tinggal} max={today} onChange={e=>setEndDate(e.target.value)}/><small className="field-hint">Pilih hari ini atau tanggal sebelumnya.</small></label><button className="button warning" onClick={endOccupancy} disabled={saving}><ArrowRightLeft size={17}/>{saving?'Memproses...':'Akhiri Masa Tinggal'}</button></div>}<div className="history-section"><h4>Riwayat Penghuni</h4>{detail.riwayat_hunian?.length?detail.riwayat_hunian.map(row=><div className="history-row" key={row.id}><div className="avatar"><UserPlus size={16}/></div><div><strong>{row.penghuni?.nama_lengkap}</strong><small>{row.mulai_tinggal} — {row.selesai_tinggal||'Sekarang'}</small></div>{row.penghuni?.jenis_penghuni&&<span className={`badge ${row.penghuni.jenis_penghuni}`}>{row.penghuni.jenis_penghuni}</span>}<span className={`badge ${row.aktif?'dihuni':'neutral'}`}>{row.aktif?'Aktif':'Selesai'}</span></div>):<div className="empty">Belum ada riwayat penghuni.</div>}</div><BillingHistory bills={detail.riwayat_tagihan}/></div></div>}
