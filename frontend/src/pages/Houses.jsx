...
export default function Houses() {
  ...
  return <div className="page-stack">
    ...
    {detail&&<div className="modal-layer"><div className="modal modal-large"><div className="modal-head"><div><span className="eyebrow">DETAIL RUMAH</span><h3>{detail.nomor_rumah}</h3></div><button onClick={()=>setDetail(null)}><X/></button></div><div className="detail-summary"><div><small>STATUS</small><span className={`badge ${detail.status}`}>{detail.status==='dihuni'?'Dihuni':'Tidak dihuni'}</span></div><div><small>ALAMAT</small><strong>{detail.alamat||'-'}</strong></div><div><small>PENGHUNI AKTIF</small><strong>{detail.penghuni_aktif?.penghuni?.nama_lengkap||'Belum ada'}</strong></div></div>{detail.status==='tidak_dihuni'?<button className="button primary" onClick={()=>{setOccupancyForm({penghuni_id:'',mulai_tinggal:today,catatan:''});setOccupancyModal(true)}}><UserPlus size={17}/>Tempatkan Penghuni</button>:<div className="end-occupancy"><label>Tanggal selesai<input type="date" value={endDate} min={detail.penghuni_aktif?.mulai_tinggal} onChange={e=>setEndDate(e.target.value)}/></label><button className="button warning" onClick={endOccupancy} disabled={saving}><ArrowRightLeft size={17}/>{saving?'Memproses...':'Akhiri Masa Tinggal'}</button></div>}<div className="history-section"><h4>Riwayat Penghuni</h4>{detail.riwayat_hunian?.length?detail.riwayat_hunian.map(row=><div className="history-row" key={row.id}><div className="avatar"><UserPlus size={16}/></div><div><strong>{row.penghuni?.nama_lengkap}</strong><small>{row.mulai_tinggal} — {row.selesai_tinggal||'Sekarang'}</small></div><span className={`badge ${row.aktif?'dihuni':'neutral'}`}>{row.aktif?'Aktif':'Selesai'}</span></div>):<div className="empty">Belum ada riwayat penghuni.</div>}</div></div></div>}

    {occupancyModal&&<div className="modal-layer modal-layer-top"><div className="modal"><div className="modal-head"><div><span className="eyebrow">HUNIAN BARU</span><h3>Tempatkan Penghuni</h3></div><button onClick={()=>setOccupancyModal(false)}><X/></button></div>{error&&<div className="alert error modal-alert">{error}</div>}<form className="form-grid" onSubmit={assignResident}><label className="full">Pilih penghuni<select value={occupancyForm.penghuni_id} onChange={e=>setOccupancyForm({...occupancyForm,penghuni_id:e.target.value})} required><option value="">Pilih penghuni...</option>{residents.map(person=><option value={person.id} key={person.id}>{person.nama_lengkap} — {person.jenis_penghuni}</option>)}</select></label><label>Tanggal mulai<input type="date" value={occupancyForm.mulai_tinggal} onChange={e=>setOccupancyForm({...occupancyForm,mulai_tinggal:e.target.value})} required/></label><label>Catatan<input value={occupancyForm.catatan} onChange={e=>setOccupancyForm({...occupancyForm,catatan:e.target.value})}/></label><div className="modal-actions full"><button type="button" className="button secondary" onClick={()=>setOccupancyModal(false)}>Batal</button><button className="button primary" disabled={saving}>{saving?'Menyimpan...':'Tempatkan Penghuni'}</button></div></form></div></div>}
  </div>
}
